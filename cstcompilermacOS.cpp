...
\s*=\s*\{)");
    ...
    if (compileFlag) {
        std::string exeFilenameUnix = filename.substr(0, filename.find_last_of("."));
        std::string compileCommand = "clang++ " + cppFilename + " -o " + exeFilenameUnix;
        std::cout << "\033[1;34mCompiling...\033[0m\n";
        int result = system(compileCommand.c_str());
        std::string executecommand = exeFilenameUnix;

        if (result == 0) {
            std::cout << "\033[1;32mCompilation successful!\033[0m Output: " << exeFilenameUnix << std::endl;
